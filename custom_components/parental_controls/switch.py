...
async def async_setup_entry(
    ...
    for player_id in players:
        ...
        entities.append(ParentModeSwitch(coordinator, config_entry, player_id))
...
class ParentModeSwitch(RestoreEntity, SwitchEntity):
    """Switch to enable parent mode for a device (bypass filtering + tracking)."""
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if last_state:
            ...
            is_on = False  # Default: parent mode off
        self._coordinator.restore_parent_mode(self._player_entity_id, is_on)
        self._coordinator.register_listener(self._handle_update)
    ...
    @property
    def is_on(self) -> bool:
        """Return if parent mode is active for this device."""
        return self._coordinator.is_parent_mode(self._player_entity_id)

    async def async_turn_on(self, **kwargs: Any) -> None:
        """Enable parent mode (bypass filtering and tracking)."""
        self._coordinator.set_parent_mode(self._player_entity_id, True)

    async def async_turn_off(self, **kwargs: Any) -> None:
        """Disable parent mode (resume normal monitoring)."""
        self._coordinator.set_parent_mode(self._player_entity_id, False)
